...
function ipAllowed(request) {
  ...
  return a===160 && b===226 && c>=128 && c<=143; // Vinet range 160.226.128.0/20
}
...
async function fetchBytesFromUrl(urlStr) {
  const r = await fetch(urlStr, { cf: { cacheEverything: true, cacheTtl: 900 } });
...
// MSA: page 1 fields + page 4 signature/name/date.
const MSA_FIELDS = {
  // Page 1 (index 0)
  full_name_p1: { page: 0, x: mm(32), y: mm(200), size: 12, w: mm(90) },  // "Full Name"
  passport_p1:  { page: 0, x: mm(32), y: mm(190), size: 12, w: mm(90) },  // "ID Number"
  id_p1:        { page: 0, x: mm(32), y: mm(180), size: 12, w: mm(90) },  // "Vinet Client Code"

  // Page 4 (index 3)
  full_name_p4: { page: 3, x: mm(130), y: mm(60),  size: 12, w: mm(65) }, // "Full First & Last Name"
  date_p4:      { page: 3, x: mm(130), y: mm(48),  size: 12, w: mm(40) }, // "Date"
  sig_box_p4:   { page: 3, x: mm(130), y: mm(75),  w: mm(65), h: mm(18) }, // signature image
};

// Debit Order: single page (index 0) + ID + signature/date
const DEBIT_FIELDS = {
  account_holder: { page: 0, x: mm(35), y: mm(235), size: 12, w: mm(90) },
  id_number:      { page: 0, x: mm(35), y: mm(225), size: 12, w: mm(90) },
  bank_name:      { page: 0, x: mm(35), y: mm(213), size: 12, w: mm(90) },
  account_number: { page: 0, x: mm(35), y: mm(203), size: 12, w: mm(90) },
  account_type:   { page: 0, x: mm(35), y: mm(193), size: 12, w: mm(60) },
  debit_day:      { page: 0, x: mm(35), y: mm(183), size: 12, w: mm(30) },
  id_field:       { page: 0, x: mm(35), y: mm(155), size: 12, w: mm(60) }, // "Vinet Customer Code"
  date_field:     { page: 0, x: mm(35), y: mm(165), size: 12, w: mm(40) },
  sig_box:        { page: 0, x: mm(35), y: mm(172), w: mm(65), h: mm(18) },
};
...
async function appendSecurityPage(pdf, sess, linkid) {
  ...
  const title = "Security & Audit Details";
  page.drawText(title, { x: margin, y: 842 - margin - 8, size: 18, font: bold, color: rgb(0.89, 0, 0.10) });
  ...
  lines.forEach(([k, v]) => {
    page.drawText(k + ":", { x: margin, y, size, font: bold });
    drawText(page, String(v), margin + keyW, y, { font, size, maxWidth: 595 - margin - (margin + keyW) });
    y -= 18;
  });

  page.drawLine({ start: { x: margin, y: y - 8 }, end: { x: 595 - margin, y: y - 8 }, color: rgb(0.85,0.85,0.85), thickness: 1 });
  page.drawText("This page is appended automatically by the Vinet Onboarding system to assist with audit and fraud prevention.", {
    x: margin, y: y - 28, size: 9, font, color: rgb(0.25,0.25,0.25)
  });
}
...
.logo{display:block;margin:0 auto 1em;max-width:120px} h1{color:#e2001a;text-align:center}
.row{display:flex;gap:.75em}.row>*{flex:1}
.btn{background:#e2001a;color:#fff;border:0;border-radius:.7em;padding:.6em 1em;cursor:pointer}
...
(function(){
  ...
  function step5(){
    ...
    const pad=(function(canvas){const ctx=canvas.getContext('2d');let d=false,l=null,x=false;function z(){const s=window.devicePixelRatio||1;const r=canvas.getBoundingClientRect();canvas.width=Math.floor(r.width*s);canvas.height=Math.floor(r.height*s);ctx.scale(s,s);ctx.lineWidth=2;ctx.lineCap='round';ctx.strokeStyle='#222';}z();window.addEventListener('resize',z);function p(e){const r=canvas.getBoundingClientRect();const t=e.touches&&e.touches[0];return {x:(t?t.clientX:e.clientX)-r.left,y:(t?t.clientY:e.clientY)-r.top};}function s(e){d=true;l=p(e);e.preventDefault();}function m(e){if(!d)return;const a=p(e);ctx.beginPath();ctx.moveTo(l.x,l.y);ctx.lineTo(a.x,a.y);ctx.stroke();l=a;x=true;e.preventDefault();}function eend(){d=false;l=null;}canvas.addEventListener('mousedown',s);canvas.addEventListener('mousemove',m);window.addEventListener('mouseup',eend);canvas.addEventListener('touchstart',s,{passive:false});canvas.addEventListener('touchmove',m,{passive:false});window.addEventListener('touchend',eend);return {clear(){const r=canvas.getBoundingClientRect();ctx.clearRect(0,0,r.width,r.height);x=false;},dataURL(){return canvas.toDataURL('image/png');},isEmpty(){return !x;}}})(document.getElementById('sig'));
...
export default {
  async fetch(request, env) {
    ...
    if (path === "/api/terms" && method === "GET") {
      ...
      const getText = async (u)=>{ try{ const r=await fetch(u,{cf:{cacheEverything:true,cacheTtl:300}}); return r.ok?await r.text():""; }catch{return "";} };
    ...
    // OTP send / verify (WA)
    async function sendWhatsAppTemplate(to, code, lang="en"){
      const endpoint = `https://graph.facebook.com/v20.0/${env.PHONE_NUMBER_ID}/messages`;
      const payload = { messaging_product:"whatsapp", to, type:"template",
        template:{ name: env.WHATSAPP_TEMPLATE_NAME || "vinetotp", language:{ code: env.WHATSAPP_TEMPLATE_LANG || lang },
    ...
    if (path === "/api/otp/verify" && method === "POST") {
      ...
      if (ok) {
        const sess = (await env.ONBOARD_KV.get(`onboard/${linkid}`, "json")) || {};
        await env.ONBOARD_KV.put(`onboard/${linkid}`, JSON.stringify({ ...sess, otp_verified:true, otp_verified_time: Date.now() }), { expirationTtl: 86400 });
    ...
    if (path === "/info/eft" && method === "GET") {
      ...
      return new Response(await (async function renderEFTPage(id) {
      ...
      })(id), { headers: { "content-type": "text/html; charset=utf-8" } });
    }
