...
// Onboarding flow + Admin + Pure-generated PDFs (MSA/DO) with terms, uploads, and security page
...
// Terms sources (plain text). Can be overridden via env vars.
...
// Admin IP allowlist (VNET range 160.226.128.0/20)
...
// network helpers
async function getTextFrom(urlStr) {
  try {
...
function drawText(page, text, x, y, opts) {
  const { font, size = 12, color = rgb(0,0,0), maxWidth = null, lineHeight = 1.35 } = opts || {};
  if (!text) return y;
  const words = String(text).split(/\s+/);
  let line = "", cy = y;
  const drawLine = (t) => page.drawText(t, { x, y: cy, size, font, color });
  if (!maxWidth) { drawLine(String(text)); return cy - size * lineHeight; }
...
function drawLabelValue(page, label, value, x, y, font, opts={}) {
  const sizeLbl = opts.sizeLbl || 11;
  const sizeVal = opts.sizeVal || 12;
  const gap = opts.gap || 3;
  page.drawText(label, { x, y, size: sizeLbl, font, color: rgb(0.35,0.35,0.35) });
  return drawText(page, value, x, y - (sizeLbl + gap), { font, size: sizeVal, color: rgb(0,0,0), maxWidth: opts.w || null });
}
async function embedLogo(pdf) {
  // tiny cache friendly fetch
  try {
    const r = await fetch(LOGO_URL, { cf: { cacheEverything: true, cacheTtl: 600 } });
    if (!r.ok) return null;
    const ab = await r.arrayBuffer();
    // try png first else jpeg
    try { return await pdf.embedPng(ab); } catch { return await pdf.embedJpg(ab); }
  } catch { return null; }
}

// ---------- Splynx helpers (only used for OTP msisdn lookup here) ----------
...
// ---------- Pure-generated PDFs ----------
async function genSecurityPage(pdf, font, sess, linkid) {
  const page = pdf.addPage([595, 842]); // A4
  const margin = 44;
  const title = "VINET — Agreement Security Summary";
  page.drawText(title, { x: margin, y: 842 - margin - 4, size: 18, font, color: rgb(0.88, 0.0, 0.10) });
  ...
  const lines = [
    ...
    ["Timestamp", new Date(sess.last_time || Date.now()).toISOString().replace("T"," ").slice(0,16)],
  ];
  let y = 842 - margin - 34;
  lines.forEach(([k,v])=>{
    page.drawText(`${k}:`, { x: margin, y, size: 11, font, color: rgb(0.35,0.35,0.35) });
    page.drawText(String(v||""), { x: margin+140, y, size: 12, font, color: rgb(0,0,0) });
    y -= 18;
  });
  page.drawText("This page is appended for audit purposes and should accompany the agreement.", {
    x: margin, y: margin, size: 10, font, color: rgb(0.45,0.45,0.45)
  });
}

function placeImageFit(page, img, box) {
  const { x, y, w, h } = box; // w,h are max box size
  const m = 10;
  const maxW = w - m*2;
  const maxH = h - m*2;
  const sz = img.scale(1);
  let drawW = maxW, drawH = (sz.height/sz.width)*drawW;
  if (drawH > maxH) { drawH = maxH; drawW = (sz.width/sz.height)*drawH; }
  const ox = x + (w - drawW)/2;
  const oy = y + (h - drawH)/2;
  page.drawImage(img, { x: ox, y: oy, width: drawW, height: drawH });
}
...
async function renderMSA(env, linkid) {
  ...
  // Page 1 — Title + Customer info + signature
  {
    ...
    if (logo) {
      ...
      p.drawImage(logo, { x: (595-w)/2, y: 842 - margin - h, width: w, height: h });
    }
    p.drawLine({ start:{x:margin,y:842- margin - 90}, end:{x:595-margin,y:842- margin - 90}, thickness:3, color: rgb(0.88,0.0,0.10) });
    p.drawText("Master Service Agreement", { x: margin, y: 842 - margin - 120, size: 20, font: fontB, color: rgb(0.88,0.0,0.10) });

    let y = 842 - margin - 160;
    y = drawLabelValue(p, "Full name", e.full_name || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "ID / Passport", e.passport || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Vinet Client Code", idOnly, margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Email", e.email || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Phone", e.phone || "", margin, y, font, { w: 595 - margin*2 });
    y -= 6;
    p.drawText("Address", { x: margin, y, size: 11, font, color: rgb(0.35,0.35,0.35) });
    y = drawText(p, [e.street, e.city, e.zip].filter(Boolean).join(", "), margin, y - 14, { font, size: 12, maxWidth: 595 - margin*2 });

    // Signature + date
    const sigY = 160;
    p.drawText("Signature:", { x: margin, y: sigY + 30, size: 12, font });
    if (sess.agreement_sig_key) {
      const sigBytes = await fetchR2Bytes(env, sess.agreement_sig_key);
      if (sigBytes) {
        const sigImg = await pdf.embedPng(sigBytes);
        placeImageFit(p, sigImg, { x: margin, y: sigY, w: 260, h: 40 });
      }
    }
    p.drawText(`Date: ${dateStr}`, { x: margin+300, y: sigY + 12, size: 12, font });
  }

  // Terms pages (Service)
  {
    const terms = await getTextFrom(env.TERMS_SERVICE_URL || DEFAULT_TERMS_SERVICE);
    const text = terms || "Terms not available.";
    const lines = text.replace(/\r/g,"").split("\n");
    const margin = 56;
    let page = pdf.addPage([595,842]);
    let y = 842 - margin;
    page.drawText("Service Terms", { x: margin, y, size: 14, font: fontB, color: rgb(0.2,0.2,0.2) });
    y -= 22;
    const maxW = 595 - margin*2;
    for (const raw of lines) {
      const t = raw.trim() ? raw : " ";
      const nextY = drawText(page, t, margin, y, { font, size: 11, maxWidth: maxW, lineHeight: 1.32 });
      if (nextY < margin + 40) {
  ...
  // ID Document page (first uploaded file)
  {
    const idUpload = (Array.isArray(sess.uploads) ? sess.uploads : []).find(u => /id/i.test(u?.label||"")) || (sess.uploads||[])[0];
    if (idUpload) {
      const obj = await env.R2_UPLOADS.get(idUpload.key);
      if (obj) {
        const ab = await obj.arrayBuffer();
        let img;
        try { img = await pdf.embedPng(ab); } catch { try { img = await pdf.embedJpg(ab); } catch {} }
        if (img) {
          const p = pdf.addPage([595,842]);
          const margin = 56;
          p.drawText("ID Document", { x: margin, y: 842 - margin, size: 14, font: fontB, color: rgb(0.2,0.2,0.2) });
          placeImageFit(p, img, { x: margin, y: 100, w: 595 - margin*2, h: 842 - margin*2 - 60 });
        }
      }
    }
  }

  // Proof of Address page (second uploaded file)
  {
    const poaUpload = (Array.isArray(sess.uploads) ? sess.uploads : []).find(u => /proof.*address/i.test(u?.label||"")) || (sess.uploads||[])[1];
    if (poaUpload) {
      const obj = await env.R2_UPLOADS.get(poaUpload.key);
      if (obj) {
        const ab = await obj.arrayBuffer();
        let img;
        try { img = await pdf.embedPng(ab); } catch { try { img = await pdf.embedJpg(ab); } catch {} }
        if (img) {
          const p = pdf.addPage([595,842]);
          const margin = 56;
          p.drawText("Proof of Address", { x: margin, y: 842 - margin, size: 14, font: fontB, color: rgb(0.2,0.2,0.2) });
          placeImageFit(p, img, { x: margin, y: 100, w: 595 - margin*2, h: 842 - margin*2 - 60 });
        }
      }
    }
  ...
  await genSecurityPage(pdf, font, sess, linkid);
...
async function renderDebit(env, linkid) {
  ...
  // Page 1 — Debit order details
  {
    ...
    p.drawLine({ start:{x:margin,y:842- margin - 90}, end:{x:595-margin,y:842- margin - 90}, thickness:3, color: rgb(0.88,0.0,0.10) });
    p.drawText("Debit Order Instruction", { x: margin, y: 842 - margin - 120, size: 20, font: fontB, color: rgb(0.88,0.0,0.10) });

    let y = 842 - margin - 160;
    y = drawLabelValue(p, "Account Holder Name", d.account_holder || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Account Holder ID / Passport", d.id_number || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Bank", d.bank_name || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Bank account no", d.account_number || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Bank account type", d.account_type || "", margin, y, font, { w: 595 - margin*2 });
    y = drawLabelValue(p, "Debit order date", d.debit_day || "", margin, y, font, { w: 595 - margin*2 });

    p.drawText(`Vinet Client Code: ${idOnly}`, { x: margin, y: 150, size: 12, font });
    p.drawText(`Date: ${dateStr}`, { x: margin, y: 132, size: 12, font });

    p.drawText("Signature:", { x: margin, y: 104, size: 12, font });
    if (sess.debit_sig_key) {
      const sigBytes = await fetchR2Bytes(env, sess.debit_sig_key);
      if (sigBytes) {
        const sigImg = await pdf.embedPng(sigBytes);
        placeImageFit(p, sigImg, { x: margin+70, y: 92, w: 260, h: 40 });
      }
    }
  }
  ...
  // Terms (Debit)
  {
    const terms = await getTextFrom(env.TERMS_DEBIT_URL || DEFAULT_TERMS_DEBIT);
    const text = terms || "Debit Order Terms not available.";
    const lines = text.replace(/\r/g,"").split("\n");
    const margin = 56;
    let page = pdf.addPage([595,842]);
    let y = 842 - margin;
    page.drawText("Debit Order Terms", { x: margin, y, size: 14, font: fontB, color: rgb(0.2,0.2,0.2) });
    y -= 22;
    const maxW = 595 - margin*2;
    for (const raw of lines) {
      const t = raw.trim() ? raw : " ";
...
<div class="card">
  ...
  <div style="margin-top:14px"><a href="/admin" class="btn">Go to Admin dashboard</a></div>
</div>
<script>
...
body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,'Helvetica Neue',Arial,sans-serif;background:#fafbfc}
.card{background:#fff;max-width:760px;margin:48px auto;border-radius:16px;box-shadow:0 2px 12px #0002;padding:22px 26px;text-align:center}
h1{color:#e2001a}.logo{height:72px;margin:0 auto 8px;display:block}.note{color:#666}
</style></head><body>
<div class="card"><img class="logo" src="${LOGO_URL}" alt="Vinet"><h1>Admin — Restricted</h1><p class="note">Access is limited to the VNET network.</p></div></body></html>`;
  }
...
(()=> {
  ...
  tabs.forEach(t => t.onclick = () => {
    ...
    t.classList.add('active'); load(t.getAttribute('data-tab'));
  });
  load('gen');
...
export default {
  async fetch(request, env) {
    ...
    if (path === "/api/terms" && method === "GET") {
      ...
      const svcUrl = env.TERMS_SERVICE_URL || DEFAULT_TERMS_SERVICE;
      const debUrl = env.TERMS_DEBIT_URL   || DEFAULT_TERMS_DEBIT;
      const service = await getTextFrom(svcUrl);
      const debit = await getTextFrom(debUrl);
      let body = "";
    ...
    // Session fetch/save (captures light audit info)
    ...
    if (path.startsWith("/api/progress/") && method === "POST") {
      ...
      const last_ip = getIP();
      const device_id = existing.device_id || (await crypto.subtle.digest("SHA-256",
        new TextEncoder().encode([last_ip, cf.asn||"", (linkid||"").slice(0,8)].join("|")))
        .then(h=>Array.from(new Uint8Array(h)).slice(0,12).map(x=>x.toString(16).padStart(2,"0")).join("")));
      const next = { ...existing, ...body, last_ip, last_ua: request.headers.get("user-agent") || "", last_loc, device_id, last_time: Date.now() };
    ...
    // WhatsApp OTP send/verify (with fallback error messages only; sending depends on env token)
    if (path === "/api/otp/send" && method === "POST") {
      ...
      try {
        // Prefer template if configured on Meta side
    ...
    if (path.startsWith("/agreements/pdf/") && method === "GET") {
      ...
      try {
        if (type === "msa") return await renderMSA(env, linkid);
        if (type === "debit") return await renderDebit(env, linkid);
    ...
    // Splynx profile proxy (for prefill only)
    if (path === "/api/splynx/profile" && method === "GET") {
      ...
      try {
        ...
        const street = src.street || src.address || src.address_1 || src.street_1 ||
                       pickFrom(src, ["street","address","address_1","street_1"]) || pickFrom(custInfo, ["street","address","address_1","street_1"]) || "";
