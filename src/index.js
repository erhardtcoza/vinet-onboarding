...
// FIX: upload with required doc meta so Splynx stops returning "Source is required"
async function splynxUploadDoc(env, whoType, id, filename, bytes, contentType, opts = {}) {
  const fd = new FormData();
  fd.set('file', new Blob([bytes], { type: contentType || 'application/pdf' }), filename);

  if (opts.type != null) fd.set('type', String(opts.type)); // e.g. "contract" or numeric code expected by your instance
  if (opts.title != null) fd.set('title', String(opts.title));
  if (opts.visible_by_customer != null) fd.set('visible_by_customer', opts.visible_by_customer ? '1' : '0');

  const ep = (whoType === 'lead')
...
async function fetchProfileForDisplay(env, id) {
  ...
  const passport =
    ...
    pickFrom(src, ["passport","id_number","idnumber","national_id","identity","identity_number","document","id_card","document_number"]) || "";
...
// --- Field positions (A4 ~ 595x842pt; origin bottom-left) ---
const MSA_FIELDS = {
  // Page 1 (index 0)
...
async function renderMsaPdf(env, linkid, bbox) {
  ...
  // Page 1 signature (duplicate final signature)
  ...
  // Page 4 full name/date/signature
...
// --- Admin UI (simple & unchanged layout) ---
...
// --- Onboarding UI (simple, no layout surprises) ---
...
<style>
  ...
  .field{margin:1em 0} input,select,textarea{width:100%;padding:.7em;font-size:1em;border-radius:.5em;border:1px solid #ddd}
  .note{font-size:12px;color:#666}
  .termsbox{max-height:220px;overflow:auto;border:1px solid #eee;padding:10px;border-radius:8px;background:#fff}
  .bigchk input{width:20px;height:20px;transform:translateY(3px);margin-right:8px}
</style></head><body>
...
(function(){
  ...
  function render(){ [step0,step1,step2,step3,step5,step6][step](); }
  render(); load();
})();
</script>
</body></html>`;
...
export default {
  async fetch(request, env) {
    ...
    // Debit save + signature (kept for completeness)
    ...
    // Admin: list onboarding sessions (lightweight)
    ...
    // Admin: get a single session (returns uploads list)
    ...
    // Admin: download uploaded file (clickable in Review UI)
    ...
    if (path.startsWith("/api/admin/push/") && method === "POST") {
      ...
      // Patch core data (+ mirror billing_email if missing)
      const data = { ...(sess.edits || {}) };
      if (data.email && !data.billing_email) data.billing_email = data.email;
      ...
      try {
        ...
        await splynxUploadDoc(env, type, idOnly, `MSA_${idOnly}.pdf`, msaBytes, 'application/pdf', {
          type: env.SPLYNX_DOC_TYPE_MSA ?? 'contract',
          title: 'MSA',
          visible_by_customer: true
        });
      ...
      try {
        if (sess.debit_sig_key) {
          ...
          await splynxUploadDoc(env, type, idOnly, `Debit_Order_${idOnly}.pdf`, debBytes, 'application/pdf', {
            type: env.SPLYNX_DOC_TYPE_DEBIT ?? 'contract',
            title: 'Debit Order',
            visible_by_customer: true
          });
        }
      ...
      try {
        ...
        for (const o of (files.objects || [])) {
          ...
          await splynxUploadDoc(env, type, idOnly, name, arr, obj.httpMetadata?.contentType || "application/octet-stream", {
            type: env.SPLYNX_DOC_TYPE_UPLOAD ?? 'scan',
            title: name,
            visible_by_customer: false
          });
        }
      } catch (e) { /* ignore */ }
