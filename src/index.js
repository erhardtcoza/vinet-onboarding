// index.js — Vinet Onboarding Worker (Times + red headings, /admin/crm/leads fix, KV→D1 on approval)
//
...
// - PDF generation for MSA & Debit (pdf-lib) — Times Roman, titles in red [1,0,0], body black (default)
// - Splynx endpoints corrected to /admin/crm/leads/...
// - KV-first sessions; on approval, record is inserted into D1 and KV marked approved
...
const RED = [1, 0, 0]; // headings/titles

// ---------- Helpers ----------
function ipAllowed(request) {
  // Restrict admin access to 160.226.128.0/20
...
function drawDashedLine(page, x1, y, x2, dash = 6, gap = 4, thickness = 1) {
  page.setLineWidth(thickness);
  page.setDash(dash, { phase: 0, dashArray: [dash, gap] });
  page.drawLine({ start: { x: x1, y }, end: { x: x2, y } }); // black by default
  page.setDash(0);
}
async function drawHeader(env, pdf, page, opts) {
  ...
  try {
    ...
    page.drawText("VINET", { x: MARGIN, y: y - 20, size: 24, font: bold });
    y -= 28;
  }
  // website + tel (black default)
  page.drawText(sanitizeAnsi(web), { x: MARGIN, y, size: 10, font });
  page.drawText(sanitizeAnsi(tel), { x: MARGIN + 180, y, size: 10, font });

  const titleY = y - 18;
  page.drawText(sanitizeAnsi(title), { x: MARGIN, y: titleY, size: 18, font: bold, color: RED });

  drawDashedLine(page, MARGIN, titleY - 10, A4W - MARGIN, 5, 3, 1);
  return titleY - 24;
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (y < bottomY) {
      if (x + colWidth + colGap <= A4W - margin - 1) {
        ...
        page.drawText("Vinet Internet Solutions", { x: margin, y: A4H - 28, size: 10, font });
        drawDashedLine(page, margin, A4H - 36, A4W - margin, 4, 3, 1);
        x = margin;
        y = A4H - 54;
      }
    }
    page.drawText(L, { x, y, size: fontSize, font }); // black default
    y -= lineGap;
  }
  return { page, x, y };
}
...
async function renderDebitPdf(env, linkid) {
  ...
  function drawRow(label, value, x, y0) {
    ...
    page.drawText(L, { x, y: y0, size: 10, font: bold });
    page.drawText(V, { x: x + 135, y: y0, size: 10, font });
    return y0 - 14;
  }
  ...
  let yR = y;
  page.drawText("Debit Order Details", { x: rightX, y: yR, size: 12, font: bold });
  yR -= 16;
  ...
  drawDashedLine(page, MARGIN, barY, A4W - MARGIN, 5, 3, 1);

  let termsY = barY - 12;
  const lines = wrapToLines(terms, font, 8, A4W - MARGIN * 2);
  for (const L of lines) {
    if (termsY < 110) break;
    page.drawText(L, { x: MARGIN, y: termsY, size: 8, font });
    termsY -= 10;
  }
  ...
  page.drawText("Name:", { x: MARGIN, y: sigTop, size: 10, font: bold });
  page.drawText(sanitizeAnsi(e.full_name || ""), { x: MARGIN + 50, y: sigTop, size: 10, font });
  page.drawLine({ start: { x: MARGIN + 50, y: sigTop - 4 }, end: { x: MARGIN + third - 10, y: sigTop - 4 } });

  page.drawText("Signature:", { x: MARGIN + third + 10, y: sigTop, size: 10, font: bold });
  try {
  ...
  page.drawLine({ start: { x: MARGIN + third + 80, y: sigTop - 4 }, end: { x: MARGIN + third * 2 - 10, y: sigTop - 4 } });

  const today = todayZA();
  page.drawText("Date (DD/MM/YYYY):", { x: MARGIN + third * 2 + 10, y: sigTop, size: 10, font: bold });
  const ddmmyyyy = toDDMMYYYY(today);
  page.drawText(ddmmyyyy, { x: MARGIN + third * 2 + 10, y: sigTop - 16, size: 10, font });
  page.drawLine({ start: { x: MARGIN + third * 2 + 10, y: sigTop - 4 }, end: { x: A4W - MARGIN, y: sigTop - 4 } });

  const page2 = pdf.addPage([A4W, A4H]);
  const y2 = await drawHeader(env, pdf, page2, { title: "Security Audit" });
  page2.drawText("— (Intentionally left for audit trail) —", { x: MARGIN, y: y2, size: 10, font });
...
async function renderMSAPdf(env, linkid) {
  ...
  drawDashedLine(page, MARGIN, afterInfoY, A4W - MARGIN, 5, 3, 1);
  ...
  page.drawText("Name:", { x: MARGIN, y: sigBaseY, size: 10, font: bold });
  page.drawText(sanitizeAnsi(e.full_name || ""), { x: MARGIN + 50, y: sigBaseY, size: 10, font });
  page.drawLine({ start: { x: MARGIN + 50, y: sigBaseY - 4 }, end: { x: MARGIN + third - 10, y: sigBaseY - 4 } });

  page.drawText("Signature:", { x: MARGIN + third + 10, y: sigBaseY, size: 10, font: bold });
  try {
  ...
  page.drawLine({ start: { x: MARGIN + third + 80, y: sigBaseY - 4 }, end: { x: MARGIN + third * 2 - 10, y: sigBaseY - 4 } });

  const today = todayZA();
  page.drawText("Date (DD/MM/YYYY):", { x: MARGIN + third * 2 + 10, y: sigBaseY, size: 10, font: bold });
  const ddmmyyyy = toDDMMYYYY(today);
  page.drawText(ddmmyyyy, { x: MARGIN + third * 2 + 10, y: sigBaseY - 16, size: 10, font });
  page.drawLine({ start: { x: MARGIN + third * 2 + 10, y: sigBaseY - 4 }, end: { x: A4W - MARGIN, y: sigBaseY - 4 } });
  ...
  async fetch(request, env) {
    ...
    if (path === "/api/admin/approve" && method === "POST") {
      ...
      try {
        ...
        const e = sess.edits || {};
        await env.DB.prepare(`
          INSERT INTO leads (splynx_id, full_name, email, phone, street, city, zip, passport, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(
          String(sess.id || "").trim(),
          e.full_name || "",
