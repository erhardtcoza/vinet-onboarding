...
// • Admin link-gen: accept POST *and* GET (/api/admin/genlink?id=...) for compatibility.
// • PDFs: Long Terms are flowed into the MSA with 2 columns over multiple pages (header repeats).
// • Debit terms text ~4pt smaller (9pt).
// • Signatures row at the *bottom* of the last content page; auto-new-page if needed.
// • Final page = Security Audit summary.
//
...
//
// Access to "/" is still IP-gated to 160.226.128.0/20.
...
function pickFrom(obj, keys){ if(!obj) return null; const want=keys.map(k=>String(k).toLowerCase()); const stack=[obj]; while(stack.length){ const cur=stack.pop(); if(Array.isArray(cur)){ for(const it of cur) stack.push(it); continue; } if(cur && typeof cur==="object"){ for(const [k,v] of Object.entries(cur)){ if(want.includes(String(k).toLowerCase())){ const s=String(v??"").trim(); if(s) return s; } if(v && typeof v==="object") stack.push(v); } } } return null; }
async function fetchProfileForDisplay(env, id){
  ...
  const passport=(custInfo&&(custInfo.passport||custInfo.id_number||custInfo.identity_number))||src.passport||src.id_number||pickFrom(src,["passport","id_number","idnumber","national_id","id_card","identity","identity_number","document_number"])||"";
...
// ---------- Admin UI (classic) ----------
function renderAdminPage(){ return `<!doctype html><html><head><meta charset="utf-8"/><title>Admin</title><meta name="viewport" content="width=device-width,initial-scale=1"/>
...
</head><body><header><img src="${LOGO_URL}" alt="Vinet"/><h1>Admin Dashboard</h1></header><div class="wrap"><div class="tabs"><div class="tab active" data-tab="gen">1. Generate onboarding link</div><div class="tab" data-tab="staff">2. Generate verification code</div><div class="tab" data-tab="inprog">3. Pending (in-progress)</div><div class="tab" data-tab="pending">4. Completed (awaiting approval)</div><div class="tab" data-tab="approved">5. Approved</div></div><div id="content">Loading…</div></div><script src="/static/admin.js"></script></body></html>`; }
function adminJs(){ return `(()=>{const tabs=[...document.querySelectorAll('.tab')]; const content=document.getElementById('content'); tabs.forEach(t=>t.onclick=()=>{tabs.forEach(x=>x.classList.remove('active')); t.classList.add('active'); load(t.getAttribute('data-tab'));}); load('gen'); const node=html=>{const d=document.createElement('div'); d.innerHTML=html; return d;};
async function load(which){
  if(which==='gen'){ content.innerHTML=''; const v=node('<div class="row"><label>Splynx Lead/Customer ID</label><input id="id" type="text" placeholder="e.g. 319"/></div><button id="go" class="primary">Generate</button><div id="out" class="muted" style="margin-top:10px"></div>');
    v.querySelector('#go').onclick=async()=>{ const id=v.querySelector('#id').value.trim(); const out=v.querySelector('#out'); if(!id){ out.textContent='Please enter an ID.'; return; } out.textContent='Working...';
      try{
        ...
        if(!r.ok){ // Fallback to GET for older flows
          r=await fetch('/api/admin/genlink?id='+encodeURIComponent(id));
        }
        const d=await r.json().catch(()=>({}));
        out.innerHTML = d.url ? ('Onboarding link: <a href="'+d.url+'" target="_blank" rel="noreferrer">'+d.url+'</a>') : ('Error generating link.'+(d.error?(' '+d.error):''));
      }catch{ out.textContent='Network error.'; }
    };
    content.appendChild(v); return;
  }
  if(which==='staff'){ content.innerHTML=''; const v=node('<div class="row"><label>Onboarding Link ID (e.g. 319_ab12cd)</label><input id="linkid" type="text" placeholder="319_ab12cd"/></div><button id="go" class="primary">Generate staff code</button><div id="out" class="muted" style="margin-top:10px"></div>');
    v.querySelector('#go').onclick=async()=>{ const linkid=v.querySelector('#linkid').value.trim(); const out=v.querySelector('#out'); if(!linkid){ out.textContent='Enter linkid'; return; } out.textContent='Working...';
      try{
  ...
  if(['inprog','pending','approved'].includes(which)){ content.innerHTML='Loading...'; try{ const r=await fetch('/api/admin/list?mode='+which); const d=await r.json(); const rows=(d.items||[]).map(i=>'<tr><td>'+i.id+'</td><td><code>'+i.linkid+'</code></td><td>'+new Date(i.updated).toLocaleString()+'</td><td><a href="/onboard/'+i.linkid+'" target="_blank">Open</a></td></tr>').join('')||'<tr><td colspan="4">No records.</td></tr>'; content.innerHTML='<table><thead><tr><th>Splynx ID</th><th>Link ID</th><th>Updated</th><th>Action</th></tr></thead><tbody>'+rows+'</tbody></table>'; }catch{ content.innerHTML='Failed to load.'; } return; }
}})();`; }

// ---------- Onboarding UI (unchanged steps) ----------
function renderOnboardPage(linkid){ return `<!doctype html><html><head><meta charset="utf-8"/><title>Vinet Onboarding</title><meta name="viewport" content="width=device-width,initial-scale=1"/>
...
</head><body><header><img src="${LOGO_URL}" alt="Vinet"/><h1>Client Onboarding</h1></header>
<div class="wrap">
  ...
  <div class="card step active" id="s1"><h2>Step 1: Verify</h2><p>Enter the 6-digit WhatsApp code or a staff code.</p><div class="flex"><input id="otp" placeholder="Enter code"/><button id="btnSend">Resend</button><button id="btnVerify" class="primary">Verify</button></div><div id="otpMsg" class="hint"></div></div>
  <div class="card step" id="s2"><h2>Step 2: Confirm details</h2>
  ...
  <div class="card step" id="s3"><h2>Step 3: Payment method</h2><div class="row"><div>Choose</div><div><select id="pay"><option value="eft">EFT</option><option value="debit">Debit Order</option></select></div></div><div id="payInfo" class="hint"></div><div style="margin-top:10px"><button id="to4" class="primary">Continue</button></div></div>
  <div class="card step" id="s4"><h2>Step 4: Upload documents</h2><div class="grid2"><div><div>ID Document (max 5MB)</div><input type="file" id="file_id"/></div><div><div>Proof of Address (optional, max 5MB)</div><input type="file" id="file_poa"/></div></div><div class="hint">JPEG/PNG/PDF accepted.</div><div style="margin-top:10px"><button id="to5" class="primary">Continue</button></div></div>
  <div class="card step" id="s5"><h2>Step 5: Sign agreements</h2>
    <div class="grid2"><div><div>MSA Signature</div><canvas id="sig1" width="500" height="180"></canvas><div class="hint">Draw with your mouse or finger.</div><button id="clear1">Clear</button></div><div><div>Debit Order (if selected)</div><canvas id="sig2" width="500" height="180"></canvas><div class="flex"><input id="agreeDebit" type="checkbox"/><label for="agreeDebit"> I authorize debit orders from my bank account.</label></div><button id="clear2">Clear</button></div></div>
    <div style="margin-top:10px"><button id="to6" class="primary">Generate PDFs</button></div><div id="pdfLinks" class="hint" style="margin-top:10px"></div></div>
  <div class="card step" id="s6"><h2>All done</h2><p>You can download your agreements above. Our team will review and activate your service.</p><a id="eftLink" target="_blank" rel="noreferrer">View EFT details</a></div>
</div>
<script>
...
$('#btnVerify').onclick = async () => {
  ...
  if(d.ok){ $('#otpMsg').textContent='Verified.';
    ...
    if(prof && prof.id){ $('#full_name').value = prof.full_name||''; $('#id_number').value = prof.passport||''; $('#customer_id').value = prof.id||''; $('#email').value = prof.email||''; $('#phone').value = prof.phone||''; $('#street').value = prof.street||''; $('#city').value = prof.city||''; $('#zip').value = prof.zip||''; $('#status').textContent='Verified at '+new Date().toLocaleString(); }
    S(2);
  } else { $('#otpMsg').textContent='Invalid code'; }
};
$('#to3').onclick = () => { S(3); $('#payInfo').innerHTML=''; };
$('#to4').onclick = async () => { S(4); };
$('#to5').onclick = async () => {
...
$('#to6').onclick = async () => {
  ...
  let links=''; if (msa.ok) { const blob = await msa.blob(); const url = URL.createObjectURL(blob); links += '<div><a download="MSA.pdf" href="'+url+'">Download MSA</a></div>'; }
  // Debit (if chosen)
  const pay = $('#pay').value;
  if (pay==='debit' && document.getElementById('agreeDebit').checked) {
...
// ---------- Terms (still available if UI needs it, but not re-displayed here) ----------
async function termsHandler(env, url){
  ...
  const txt = esc(await fetchText(kind==="debit" ? debUrl : svcUrl) || "");
  const hdr = kind==="debit" ? "Debit Order Terms" : "Service Terms";
  return new Response(`<h3>${hdr}</h3><pre style="white-space:pre-wrap">${txt}</pre>`, { headers:{ "content-type":"text/html; charset=utf-8" } });
}
...
function drawKV(page, fonts, x, y, kv, opts={colW:140, lineH:16, size:11}){
  ...
  let yy=y;
  for(const [k,v] of kv){ page.drawText(String(k),{x,y:yy,size,font:helvBold}); page.drawText(String(v||""),{x:x+colW,y:yy,size,font:helv}); yy-=lineH; }
  return yy;
}
function drawTwoCols(page, fonts, xL, xR, y, leftItems, rightItems, opts={lineH:16, size:11}){
  const { helv, helvBold } = fonts; const size=opts.size, lineH=opts.lineH; let yl=y, yr=y;
  for(const [k,v] of leftItems){ page.drawText(String(k),{x:xL,y:yl,size,font:helvBold}); page.drawText(String(v||""),{x:xL+120,y:yl,size,font:helv}); yl-=lineH; }
  for(const [k,v] of rightItems){ page.drawText(String(k),{x:xR,y:yr,size,font:helvBold}); page.drawText(String(v||""),{x:xR+120,y:yr,size,font:helv}); yr-=lineH; }
  return Math.min(yl, yr);
}
function flowTwoColumnText(doc, fonts, logo, title, rawText, size){
  // Returns { pages:[page,...], lastY:number } with pages already created & header drawn
  ...
  function newPage(){
    ...
    pages.push(page);
    x = marginX; y = A4.h - marginTop; col = 0; line="";
  }
  function newColumn(){
    col = 1; x = marginX + colWidth + colGap; y = A4.h - marginTop; line="";
  }
...
async function buildPDF(docType, data, request){
  ...
  if (docType === "debit") {
    ...
    // MSA: left client info, right address
  ...
  // Ensure space for signatures; if not, new page
  if (y < 140) {
    page = doc.addPage([A4.w, A4.h]);
    drawHeader(page, fonts, logo, title);
    y = A4.h - 140;
...
export default {
  async fetch(request, env){
    ...
    if (path === "/info/eft" && method === "GET") {
      ...
      return new Response(await (async function(){return `<!doctype html><html><head><meta charset="utf-8"/><title>EFT</title><meta name="viewport" content="width=device-width,initial-scale=1"/></head><body>EFT details for ${esc(id)}</body></html>`})(), { headers: { "content-type":"text/html; charset=utf-8" } });
    }

    // Admin: generate onboarding link (POST preferred, GET supported)
    ...
    // Terms API (if UI wants it)
    if (path === "/api/terms" && method === "GET") return termsHandler(env, url);

    // PDFs
    if (path === "/api/pdf/msa" && method === "POST") {
      const body = await request.json();
      const resp = await buildPDF("msa", { ...body, terms_service: await fetchText(env.TERMS_SERVICE_URL || "https://onboarding-uploads.vinethosting.org/vinet-master-terms.txt") }, request);
    ...
    if (path === "/api/pdf/debit" && method === "POST") {
      ...
      const resp = await buildPDF("debit", { ...body, terms_debit: await fetchText(env.TERMS_DEBIT_URL || "https://onboarding-uploads.vinethosting.org/vinet-debitorder-terms.txt") }, request);
      await markStatus(env, body.customer_id || "unknown", body.linkid || "", "pending");
      return resp;
    }
