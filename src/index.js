// index.js — Vinet Onboarding Worker (safe wrapper build)
// - Same endpoints as before
// - Onboarding page JS wrapped with a try/catch that surfaces errors on-screen
// - /api/progress/fault added to log client-side crashes into KV
...
<style>
  ...
  .card{max-width:1100px;margin:2.2em auto;background:#fff;border-radius:18px;box-shadow:0 4px 18px #0002;padding:18px 20px}
...
function adminJs() {
  return `(()=> {
    ...
    async function load(which){
      if(which==='gen'){
        content.innerHTML='';
        const v=H('<div class="field"><label>Splynx Lead/Customer ID</label><div class="row"><input id="id"/><button class="btn" id="go">Generate</button></div></div><div id="out" class="field"></div>');
        v.querySelector('#go').onclick=async()=>{
          ...
          try{
            ...
            if(d.url){
              out.innerHTML='<div style="padding:.8em;border:1px solid #eee;border-radius:12px;background:#fafafa">Onboarding link: <a href="'+d.url+'" target="_blank">'+d.url+'</a></div>';
            } else out.textContent='Error generating link.';
      ...
      if(which==='staff'){
        content.innerHTML='';
        const v=H('<div class="field"><label>Onboarding Link ID (e.g. 319_ab12cd34)</label><div class="row"><input id="linkid"/><button class="btn" id="go">Generate staff code</button></div></div><div id="out" class="field note"></div>');
        v.querySelector('#go').onclick=async()=>{
      ...
      if(['inprog','pending','approved'].includes(which)){
        ...
        try{
          ...
          const rows=(d.items||[]).map(i =>
            '<tr><td>'+i.id+'</td><td>'+i.linkid+'</td><td>'+new Date(i.updated).toLocaleString()+'</td>'+
            '<td>'+(which==='pending'
              ? '<a class="btn" href="/admin/review?linkid='+encodeURIComponent(i.linkid)+'">Review</a>'
              : '<a class="btn-outline" href="/onboard/'+i.linkid+'" target="_blank">Open</a>')+'</td></tr>'
...
// ---------- Onboarding HTML (with error wrapper) ----------
...
<style>
  :root{--brand:#e2001a;}
  body{font-family:system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,sans-serif;background:#fafbfc;color:#232}
  .card{background:#fff;max-width:820px;margin:2.2em auto;border-radius:18px;box-shadow:0 4px 18px #0002;padding:24px}
  .logo{display:block;margin:0 auto 8px;max-width:220px}
  ...
  .thin{max-width:680px;margin-inline:auto}
</style>
</head>
<body>
<div class="card">
...
(function(){
  ...
  function showFatal(err){
    try{
      ...
      if(step){
        step.innerHTML = '<div style="padding:12px;border:1px solid #f3b;border-radius:10px;background:#fff2f6;color:#900">'+
          '<b>There was a problem loading this step.</b><br><span style="font-size:12px">'+
          (err && (err.stack || err.message || err))+'</span></div>';
      }
    }catch(_){}
    try{
  ...
  try{
    ...
    function step1(){
      stepEl.innerHTML = [
        ...
        '<div class="pill-wrap" style="margin-bottom:.6em"><span class="pill active" id="p-wa">WhatsApp OTP</span><span class="pill" id="p-staff">I have a staff code</span></div>',
        '<div id="waBox" class="field"></div>',
        '<div id="staffBox" class="field" style="display:none"></div>'
      ].join('');

      const wa=document.getElementById('waBox');
      wa.innerHTML='<div id="otpmsg" class="note" style="margin:.2em 0 .8em"></div><div class="row"><input id="otpWa" maxlength="6" pattern="\\\\d{6}" placeholder="6-digit code" required /><button class="btn" id="verifyWa">Verify</button></div><div style="margin-top:.6em"><button class="btn-outline" id="resend">Resend code</button></div>';
      sendOtp();
      document.getElementById('resend').onclick=(e)=>{ e.preventDefault(); sendOtp(); };
      document.getElementById('verifyWa').onclick=async()=>{
        const otp=document.getElementById('otpWa').value.trim();
      ...
      staff.innerHTML='<div class="note">Ask Vinet for a one-time staff code.</div><div class="row"><input id="otpStaff" maxlength="6" pattern="\\\\d{6}" placeholder="6-digit staff code" required /><button class="btn" id="verifyStaff">Verify</button></div><div id="staffMsg" class="note" style="margin-top:.4em"></div>';
      document.getElementById('verifyStaff').onclick=async()=>{
        const otp=document.getElementById('otpStaff').value.trim();
    ...
    function step2(){
      ...
      stepEl.innerHTML = [
        ...
        '<div class="row" style="margin-top:10px"><button class="btn-outline" id="back1">Back</button><button class="btn" id="cont">Continue</button></div>'
      ].join('');

      // EFT details inline
      const idOnly=(linkid||'').split('_')[0];
      const eft=document.getElementById('eftBox');
      eft.innerHTML = [
        '<div class="row">',
        ...
        '</div>',
        '<div class="field"><label><b>Reference (use this on EFT)</b></label><input readonly value="'+idOnly+'"/></div>',
        '<div style="text-align:center;"><a class="btn-outline" href="/info/eft?id='+idOnly+'" target="_blank">Print banking details</a></div>',
        '<div class="note" style="margin-top:.6em">Please make sure you use the correct reference when making EFT payments.</div>'
      ].join('');
      ...
      document.getElementById('pm-eft').onclick=()=>{ state.pay_method='eft'; document.getElementById('eftBox').style.display='block'; document.getElementById('debitBox').style.display='none'; save(); };
      document.getElementById('pm-debit').onclick=()=>{ state.pay_method='debit'; renderDebitForm(); document.getElementById('eftBox').style.display='none'; save(); };
      ...
      document.getElementById('cont').onclick=async(e)=>{
        ...
        if(state.pay_method==='debit'){
          state.debit = {
            ...
            account_type:   (document.getElementById('d_type')||{}).value||'cheque',
            debit_day:      (document.getElementById('d_day')||{}).value||'1'
          };
        }
      ...
      (async()=>{
        try{
          ...
          document.getElementById('box').innerHTML=[
            '<div class="row"><div class="field"><label>Full name</label><input id="f_full" value="'+(cur.full_name||'')+'"/></div><div class="field"><label>Phone</label><input id="f_phone" value="'+(cur.phone||'')+'"/></div></div>',
            ...
            '<div class="row"><button class="btn-outline" id="back2">Back</button><button class="btn" id="cont">Continue</button></div>'
          ].join('');
          document.getElementById('back2').onclick=(e)=>{ e.preventDefault(); step=2; state.progress=step; setProg(); save(); render(); };
          document.getElementById('cont').onclick=(e)=>{ e.preventDefault(); state.edits={ full_name:val("f_full"), email:val("f_email"), phone:val("f_phone"), street:val("f_street"), city:val("f_city"), zip:val("f_zip") }; step=4; state.progress=step; setProg(); save(); render(); };
    ...
    function step4(){
      stepEl.innerHTML=[
        ...
        '<div class="note" style="margin-bottom:.8em">Please upload your supporting documents — ID or Passport and proof of address (as per RICA regulations). Max 2 files, 5MB each.</div>',
        '<input id="u1" type="file" accept="image/*,.pdf" />',
        '<div style="height:10px"></div>',
        '<input id="u2" type="file" accept="image/*,.pdf" />',
        '<div class="row" style="margin-top:12px"><button class="btn-outline" id="back3">Back</button><button class="btn" id="cont">Continue</button></div>'
      ].join('');
      document.getElementById('back3').onclick=(e)=>{ e.preventDefault(); step=3; state.progress=step; setProg(); save(); render(); };
      document.getElementById('cont').onclick=async(e)=>{
        e.preventDefault();
        // (no-op upload here; keep UI snappy — your real /api/onboard/upload can be used)
    ...
    function step5(){
      stepEl.innerHTML=[
        ...
        '<div class="field" style="margin-top:.8em"><label><input class="check" type="checkbox" id="agreeChk"/> '+
          'I confirm the accuracy of the information contained in this Agreement and warrant that I am duly authorised to enter into an agreement with VINET on behalf of the customer/myself.'+
        '</label></div>',
        '<div class="field"><label>Draw your signature</label><canvas id="sig" class="signature"></canvas><div class="row"><button class="btn-outline" id="clearSig">Clear</button><span class="note" id="sigMsg"></span></div></div>',
        '<div class="row"><button class="btn-outline" id="back4">Back</button><button class="btn" id="signBtn">Agree & Sign</button></div>'
      ].join('');
      ...
      document.getElementById('signBtn').onclick=async(e)=>{
        ...
        if(!(document.getElementById('agreeChk')||{}).checked){ msg.textContent='Please tick the checkbox to accept.'; return; }
        ...
        try{
          ...
          const d=await r.json().catch(()=>({ok:false}));
          if(d.ok){ step=6; state.progress=step; setProg(); save(); render(); }
...
export default {
  async fetch(request, env) {
    ...
    // OTP: send / verify (template + fallback text)
    ...
    async function sendWhatsAppTextIfSessionOpen(toMsisdn, bodyText) {
      const endpoint = `https://graph.facebook.com/v20.0/${env.PHONE_NUMBER_ID}/messages`;
      const r = await fetch(endpoint, { method:"POST", headers:{ Authorization:`Bearer ${env.WHATSAPP_TOKEN}`, "Content-Type":"application/json" }, body: JSON.stringify({ messaging_product:"whatsapp", to: toMsisdn, type:"text", text:{ body:bodyText } }) });
    ...
    if (path === "/api/terms" && method === "GET") {
      ...
      const service = await httpGetText(TERMS_SERVICE_URL);
      const debit = pay === "debit" ? await httpGetText(TERMS_DEBIT_URL) : "";
    ...
    if (path === "/api/progress/fault" && method === "POST") {
      const b = await request.json().catch(()=>({}));
      try{
        await env.ONBOARD_KV.put(`fault/${Date.now()}_${Math.random().toString(36).slice(2,8)}`, JSON.stringify({
          ...b, ip:getIP(), ua:getUA(), t:Date.now()
        }), { expirationTtl: 60*60*24*7 });
      }catch{}
      return json({ ok:true });
    }

    // splynx profile proxy (tolerant to missing data)
    if (path === "/api/splynx/profile" && method === "GET") {
      const id = url.searchParams.get("id");
      if (!id) return json({ error:"Missing id" }, 400);
      try { const prof = await fetchProfileForDisplay(env, id); return json(prof); }
      catch { return json({ full_name:"", email:"", phone:"", street:"", city:"", zip:"" }); }
    }
    ...
    // Signature save (kept simple here)
