...
// unified low-level
async function splynxRequest(env, method, endpoint, body, extraHeaders) {
  const r = await fetch(`${env.SPLYNX_API}${endpoint}`, {
    method,
    headers: {
...
// robust writer: PATCH -> PUT -> legacy POST {action: "edit", params: ...}
async function splynxWRITE(env, endpoint, data) {
  try {
    return await splynxRequest(env, "PATCH", endpoint, data);
  } catch (e) {
    const msg = String(e.message || "");
    if (!/Invalid action|SERVER_ERROR|405|501|Method Not Allowed/i.test(msg)) throw e;
  }
  try {
    return await splynxRequest(env, "PUT", endpoint, data);
  } catch (e) {
    const msg = String(e.message || "");
    if (!/Invalid action|SERVER_ERROR|405|501|Method Not Allowed/i.test(msg)) throw e;
  }
  // legacy fallback supported by some installs
  return await splynxRequest(env, "POST", endpoint, { action: "edit", params: data });
}
...
async function splynxUploadDoc(env, type, id, filename, bytes, contentType) {
  ...
  const text = await r.text().catch(()=> "");
  if (!r.ok) throw new Error(`Splynx upload ${ep} ${r.status} ${text}`);
  try { return text ? JSON.parse(text) : {}; } catch { return {}; }
}
...
async function fetchProfileForDisplay(env, id) {
  ...
  const passport =
    ...
    pickFrom(src, ["passport","id_number","idnumber","national_id","identity_number","document","id_card","identity_number","document_number"]) || "";
...
export default {
  async fetch(request, env) {
    ...
    if (path.startsWith("/api/admin/push/") && method === "POST") {
      ...
      // Build patch data (mirror email -> billing_email; map address fields)
      const e = sess.edits || {};
      ...
      const corePatch = {
        ...
        ...(e.street ? { street_1: e.street, street: e.street } : {}),
        ...(e.city ? { city: e.city } : {}),
        ...(e.zip ? { zip_code: e.zip, zip: e.zip } : {}),
      };

      // Patch core data with robust writer
      try {
        if (type === "lead") await splynxWRITE(env, `/admin/crm/leads/${idOnly}`, corePatch);
        else await splynxWRITE(env, `/admin/customers/customer/${idOnly}`, corePatch);
      } catch (e) {
        return json({ ok:false, error:`patch_failed:${e.message}` }, 502);
      }

      // Update passport on customer-info if present
      try {
        if (e.passport) {
          await splynxWRITE(env, `/admin/customers/customer-info/${idOnly}`, { passport: e.passport });
        }
      } catch (_) {
        // ignore, not critical
      }
