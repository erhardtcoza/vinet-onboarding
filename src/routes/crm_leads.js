...
import { ensureLeadSchema, json, safeParseJSON } from "../utils/db.js";
import {
...
export function mount(router) {
  ...
  // Simple CRM landing (always render to avoid 1101)
  router.add("GET", "/crm", (_req) => {
    return html(`<!doctype html><meta charset="utf-8"><title>CRM</title>
<style>body{font-family:system-ui;margin:24px}</style>
<h2>CRM</h2><p>Use the <code>/admin/queue</code> dashboard and <code>/api/admin/*</code> endpoints.</p>`);
  });
  ...
  router.add("GET", "/api/admin/get", async (req, env) => {
    ...
    const row = await env.DB.prepare(
      "SELECT id, payload, processed, splynx_id FROM leads_queue WHERE id=?1"
    ).bind(id).first();
  ...
  router.add("POST", "/api/admin/match", async (req, env) => {
    const body = await req.json().catch(() => null);
    ...
    const { leadHits, custHits } = findCandidates(
      { name: p.name || p.full_name, email: p.email, phone: p.phone || p.phone_number },
      leads, customers
    );
  ...
  router.add("GET", "/api/admin/splynx/fetch", async (req) => {
    ...
    const light = rows.map((x) => ({
      id: x.id, status: x.status || "", name: x.name || "", email: x.email || "",
      phone: x.phone || "", city: x.city || "", last_contacted: x.last_contacted || "",
    }));
  ...
  router.add("POST", "/api/admin/submit", async (req, env) => {
    ...
    if (body.overwrite_id) {
      ...
      const exact = (Array.isArray(allLeads) ? allLeads : []).find(
        (l) =>
          ...
          String(l.phone || "") === String(p.phone || p.phone_number || "")
      );
      if (exact) {
    ...
    await env.DB.prepare(
      "UPDATE leads_queue SET processed=1, splynx_id=?1, status='synced' WHERE id=?2"
    ).bind(splynxId, body.id).run();

    // Optional helper: mirror to leads table if you keep it
    if (splynxId) {
      await env.DB.prepare(
        "UPDATE leads SET splynx_id=?1, synced=1 WHERE email=?2 OR phone=?3"
      ).bind(splynxId, p.email || "", p.phone || p.phone_number || "").run();
    }
  ...
  router.add("POST", "/api/admin/wa", async (req, env) => {
    ...
    if (!row) return json({ error: "Not found" }, 404);
    const p = safeParseJSON(row.payload);
    const code = `${(p.name || p.full_name || "client").split(" ")[0].toLowerCase()}_${Math.random().toString(36).slice(2, 8)}`;
    const onboardingUrl = `https://onboard.vinet.co.za/onboard/${code}`;
    try {
      await sendOnboardingTemplate(env, p.phone || p.phone_number, p.name || p.full_name, onboardingUrl, "en_US");
...
// So admin.js can delegate /api/admin/*
export async function handle(_req, _env, _ctx) {
