...
export async function handlePublic(request, env) {
  ...
  if (request.method === "POST" && pathname === "/ts-verify") {
    try {
      const { token, skip } = await request.json().catch(() => ({}));

      // Allow skip explicitly (user tapped Skip)
      if (skip === true) {
        const cookie = "ts_ok=0; Max-Age=86400; Path=/; Secure; SameSite=Lax";
        return json({ ok: true, skipped: true }, 200, { "set-cookie": cookie });
      }
      ...
      return json({ ok: true, success: !!result.success }, 200, { "set-cookie": cookie });
    } catch {
      const cookie = "ts_ok=0; Max-Age=86400; Path=/; Secure; SameSite=Lax";
      return json({ ok: true, success: false }, 200, { "set-cookie": cookie });
    }
  }
