...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .logo-wrap{width:160px;aspect-ratio:16/9;display:grid;place-items:center}
  .logo{max-width:100%;max-height:100%;object-fit:contain;image-rendering:-webkit-optimize-contrast;filter:drop-shadow(0 2px 8px #00000014);animation:breath 3s ease-in-out infinite}
  @keyframes breath{0%{transform:scale(1)}50%{transform:scale(1.035)}100%{transform:scale(1)}}
  h1{margin:.25rem 0 0;font-size:2.0rem}
  .bar{height:6px;background:#eef2f7;border-radius:999px;overflow:hidden;margin:.25rem 0 1rem;width:100%}
  .bar>i{display:block;height:100%;width:0;background:var(--red);border-radius:999px;transition:width .8s ease}
  .muted{color:var(--muted);text-align:center}
  .row{display:flex;gap:.75rem;justify-content:center;margin-top:1rem;flex-wrap:wrap}
  button{padding:.8rem 1.15rem;border-radius:999px;border:0;background:#111;color:#fff;font-weight:800;cursor:pointer}
  ...
  <main class="card">
    ...
    <div class="row">
      ...
      <button class="alt" id="skip" type="button">Skip</button>
    </div>
  </main>
<script>
(() => {
