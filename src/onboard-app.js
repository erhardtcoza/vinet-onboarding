// public/onboard-app.js
(function () {
  const { linkid } = window.__ONBOARD_CTX__ || {};
  if (!linkid) {
    console.error("Missing linkid bootstrap");
    return;
  }
  ...
  async function sendOtp() {
    ...
    try {
      ...
      if (m) m.textContent = "Network error.";
    }
  }
  ...
  function renderStep0() {
    root.innerHTML =
      '<h2>Welcome</h2>' +
      "<p>We’ll quickly verify you and capture the information required by RICA to activate your service.</p>" +
  ...
  function renderStep1() {
    ...
    staffInput.addEventListener("input", () => {
      staffInput.value = staffInput.value
        .replace(/\D+/g, "")
        .slice(0, 6);
    });
  ...
  function renderStep2() {
    root.innerHTML =
      '<h2>Confirm your details</h2><div id="box" class="note">Loading…</div>';

    (async () => {
      try {
        const id = (linkid || "").split("_")[0];
        const r = await fetch(
          "/api/splynx/profile?id=" + encodeURIComponent(id)
        );
        const p = await r.json();

        const cur = {
          full_name:
            state.edits.full_name ?? p.full_name ?? "",
          email: state.edits.email ?? p.email ?? "",
          phone: state.edits.phone ?? p.phone ?? "",
          passport:
            state.edits.passport ?? p.passport ?? "",
          street:
            state.edits.street ?? p.street ?? "",
          city: state.edits.city ?? p.city ?? "",
          zip: state.edits.zip ?? p.zip ?? "",
        };

        document.getElementById("box").innerHTML = [
          '<div class="row"><div class="field"><label>Full name</label><input id="f_full" value="' +
            (cur.full_name || "") +
            '"/></div><div class="field"><label>ID / Passport</label><input id="f_id" value="' +
            (cur.passport || "") +
            '"/></div></div>',
          '<div class="row"><div class="field"><label>Email</label><input id="f_email" value="' +
            (cur.email || "") +
            '"/></div><div class="field"><label>Phone</label><input id="f_phone" value="' +
            (cur.phone || "") +
            '"/></div></div>',
          '<div class="row"><div class="field"><label>Street</label><input id="f_street" value="' +
            (cur.street || "") +
            '"/></div><div class="field"><label>City</label><input id="f_city" value="' +
            (cur.city || "") +
            '"/></div></div>',
          '<div class="field"><label>ZIP Code</label><input id="f_zip" value="' +
            (cur.zip || "") +
            '"/></div>',
          '<div class="row"><a class="btn-outline" id="back2">Back</a><button class="btn" id="cont">Continue</button></div>',
        ].join("");

        document.getElementById("back2").onclick = (e) => {
          e.preventDefault();
          step = 1;
          state.progress = step;
          setProg();
        ...
        document.getElementById("cont").onclick = (e) => {
          ...
          state.edits = {
            full_name: document
              .getElementById("f_full")
              .value.trim(),
            email: document
              .getElementById("f_email")
              .value.trim(),
            phone: document
            ...
            passport: document
              ...
              .value.trim(),
            street: document
              .getElementById("f_street")
              .value.trim(),
            city: document
              .getElementById("f_city")
              .value.trim(),
            zip: document
              .getElementById("f_zip")
              .value.trim(),
          };
          step = 3;
          state.progress = step;
  ...
  function renderStep3() {
    ...
    document.getElementById("back3").onclick = (e) => {
      e.preventDefault();
      step = 2;
      state.progress = step;
      setProg();
    ...
    document.getElementById("next").onclick = async (e) => {
      ...
      if (!fId || !fPoa) {
        err.textContent = "Both documents are required.";
        return;
      }
      ...
      async function up(file, label) {
        ...
        const name = (file.name || label).replace(
          /[^a-z0-9_.-]/gi,
          "_"
        );
        const r = await fetch(
          "/api/onboard/upload?linkid=" +
  ...
  function renderStep4() {
    ...
    root.innerHTML = [
      ...
      '<div class="field"><div class="pill-wrap"><span class="pill ' +
        (pay === "eft" ? "active" : "") +
        '" id="pm-eft">EFT</span><span class="pill ' +
        (pay === "debit" ? "active" : "") +
        '" id="pm-debit">Debit order</span></div></div>',
      '<div id="eftBox" class="field" style="display:' +
        (pay === "eft" ? "block" : "none") +
        ';"></div>',
      '<div id="debitBox" class="field" style="display:' +
        (pay === "debit" ? "block" : "none") +
        ';"></div>',
    ...
    function renderEftBox() {
      ...
      box.innerHTML = [
        ...
        '<div class="field"><label><b>Reference</b></label><input readonly style="font-weight:900" value="' +
          id +
          '"/></div>',
        '<div class="note">Please use the correct <b>Reference</b> when making EFT payments.</div>',
        '<div style="display:flex;justify-content:center;margin-top:.6em"><a class="btn-outline" href="/info/eft?id=' +
          id +
          '" target="_blank" style="text-align:center;min-width:260px">Print banking details</a></div>',
      ].join("");
    }

    let dPad = null;
    function renderDebitFormBox() {
      const d = state.debit || {};
      const box = document.getElementById("debitBox");
      box.style.display = "block";
      box.innerHTML = [
        '<div class="row">',
        '<div class="field"><label>Bank Account Holder Name</label><input id="d_holder" value="' +
          (d.account_holder || "") +
          '" required /></div>',
        '<div class="field"><label>Bank Account Holder ID no</label><input id="d_id" value="' +
          (d.id_number || "") +
          '" required /></div>',
        "</div>",
        '<div class="row">',
        '<div class="field"><label>Bank</label><input id="d_bank" value="' +
          (d.bank_name || "") +
          '" required /></div>',
        '<div class="field"><label>Bank Account No</label><input id="d_acc" value="' +
          (d.account_number || "") +
          '" required /></div>',
        "</div>",
        '<div class="row">',
        '<div class="field"><label>Bank Account Type</label><select id="d_type"><option value="cheque" ' +
          ((d.account_type || "") === "cheque"
            ? "selected"
            : "") +
          '>Cheque / Current</option><option value="savings" ' +
          ((d.account_type || "") === "savings"
            ? "selected"
            : "") +
          '>Savings</option><option value="transmission" ' +
          ((d.account_type || "") === "transmission"
            ? "selected"
            : "") +
        ...
        '<div class="field bigchk" style="margin-top:.8em"><label style="display:flex;align-items:flex-start;gap:.55em"><input id="d_agree" type="checkbox"> <span>I agree to the Debit Order terms</span></label></div>',
      ...
      (async () => {
        try {
          const r = await fetch("/api/terms?kind=debit");
          const t = await r.text();
          document.getElementById("debitTerms").innerHTML =
            t || "Terms not available.";
        } catch {
          document.getElementById(
            "debitTerms"
          ).textContent = "Failed to load terms.";
        }
      })();

      dPad = sigPad(document.getElementById("d_sig"));
      document.getElementById("d_clear").onclick = (e) => {
        e.preventDefault();
        dPad.clear();
      };
    }

    function hideDebitForm() {
      const box = document.getElementById("debitBox");
    ...
    // Bind payment method pills
    document.getElementById("pm-eft").onclick = () => {
      state.pay_method = "eft";
      hideDebitForm();
      renderEftBox();
      save();
    };
    document.getElementById("pm-debit").onclick = () => {
      state.pay_method = "debit";
      hideEft();
      renderDebitFormBox();
      save();
    };
    ...
    // initial sub-render
    if (pay === "debit") renderDebitFormBox();
    else renderEftBox();

    // back/continue buttons
    document.getElementById("back4").onclick = (e) => {
      e.preventDefault();
      step = 3;
      state.progress = step;
      setProg();
    ...
    document.getElementById("cont").onclick = async (e) => {
      ...
      if (state.pay_method === "debit") {
        ...
        if (!document.getElementById("d_agree").checked) {
          msg.textContent =
            "Please confirm you agree to the Debit Order terms.";
          return;
        }
        if (!dPad || dPad.isEmpty()) {
          msg.textContent =
            "Please add your signature for the Debit Order.";
          return;
        }

        state.debit = {
          account_holder: document
            .getElementById("d_holder")
            .value.trim(),
          id_number: document
            .getElementById("d_id")
            .value.trim(),
          bank_name: document
            .getElementById("d_bank")
            .value.trim(),
          account_number: document
            .getElementById("d_acc")
            .value.trim(),
          account_type:
  ...
  function renderStep5() {
    root.innerHTML = [
      ...
      '<div class="field bigchk" style="margin-top:10px;"><label style="display:flex;align-items:flex-start;gap:.6em;"><input type="checkbox" id="agreeChk" /> <span>I confirm the accuracy of the information contained in this Agreement and warrant that I am duly authorised to enter into an agreement with VINET on behalf of the customer/myself.</span></label></div>',
    ...
    (async () => {
      try {
        const r = await fetch("/api/terms?kind=service");
        const t = await r.text();
    ...
    document.getElementById("clearSig").onclick = (e) => {
      e.preventDefault();
      pad.clear();
    };

    document.getElementById("back5").onclick = (e) => {
      e.preventDefault();
      step = 4;
      state.progress = step;
      setProg();
    ...
    document.getElementById("signBtn").onclick = async (e) => {
      ...
      if (!document.getElementById("agreeChk").checked) {
        msg.textContent =
          "Please tick the checkbox to confirm agreement.";
        return;
      }
      ...
      try {
        ...
        const r = await fetch("/api/sign", {
          ...
          body: JSON.stringify({ linkid, dataUrl }),
        });
        const d = await r.json().catch(() => ({ ok: false }));
        if (d.ok) {
          step = 6;
          ...
          msg.textContent =
            d.error || "Failed to save signature.";
        }
      } catch {
  ...
  function renderStep6() {
    const showDebit =
      state && state.pay_method === "debit";
    const docIcon =
      '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zM14 3.5L18.5 8H14V3.5zM8 12h8v1.5H8V12zm0 3h8v1.5H8V15zM8 9h4v1.5H8V9z"/></svg>';
    ...
    root.innerHTML = [
      ...
      '<ul class="doclist">',
      '<li class="doc-item"><span class="doc-ico">' +
        docIcon +
        '</span>' +
        '<div><a href="/pdf/msa/' +
        linkid +
        '" target="_blank">Master Service Agreement (PDF)</a>' +
        ' &nbsp;•&nbsp; <a href="/agreements/msa/' +
        linkid +
      ...
      showDebit
        ? '<li class="doc-item"><span class="doc-ico">' +
          docIcon +
          '</span><div><a href="/pdf/debit/' +
          linkid +
          '" target="_blank">Debit Order Agreement (PDF)</a>' +
          ' &nbsp;•&nbsp; <a href="/agreements/debit/' +
          linkid +
          '" target="_blank">View in browser</a></div></li>'
        : "",
      "</ul>",
      "</div>",
    ].join("");
