import { renderAdminPage, renderAdminReviewHTML } from "./ui/admin.js"; // only if needed here (we might move later)
...
export async function handlePublicRoutes({ path, method, url, env }) {
  ...
  if (path === "/info/eft" && method === "GET") {
    ...
    const html = `<!DOCTYPE html>...${escapeHtml(id)}...`; // reuse same markup you already have
  ...
  if (path.startsWith("/agreements/") && method === "GET") {
    ...
    if (type === "msa") {
      ...
      return new Response(
        `<!doctype html><meta charset="utf-8"><title>Master Service Agreement</title><h2>Master Service Agreement</h2><pre style="white-space:pre-wrap">${esc(text)}</pre>`,
        { headers: { "content-type": "text/html; charset=utf-8" } }
      );
    }
    ...
    if (type === "debit") {
      ...
      return new Response(
        `<!doctype html><meta charset="utf-8"><title>Debit Order Instruction</title><h2>Debit Order Instruction</h2><pre style="white-space:pre-wrap">${esc(text)}</pre>`,
        { headers: { "content-type": "text/html; charset=utf-8" } }
      );
    }
